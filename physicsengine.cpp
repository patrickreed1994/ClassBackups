...
const int MAXBALLS=1000;		//max balls allowed
int BallCount=0;			//number of balls in the simulation
int WallCount=0;			//number of walls in the simulation
int LinkCount=0;			//number of links in the simulation
int Bound=50;				//
int Radius=5;				//radius of balls
const float Restitution=0.9;		//elasticity of balls
const float WallRestitution=0.2;	//elasticity of walls
const float gravity=0.5;		//gravity acting on simulation
float DT=0.1;				//speed of simulation
float FRICTION=40.0;			//friction force of balls against walls

Ball* balls[MAXBALLS];			//array of balls
Wall* walls[MAXBALLS];			//array of walls
Wall* trackA[MAXBALLS];			//first track
Wall* trackB[MAXBALLS];			//second track
Link* links[MAXBALLS];			//array of links

extern QGraphicsScene* thescene;	//scene variable

bool dontdographics=FALSE;		//boolean about whether or not to include graphics
bool simulating=FALSE;			//checks if simulation is on or off

Ball* getBall(int i)
{
	return balls[i];		//fetches ball in balls array
}
